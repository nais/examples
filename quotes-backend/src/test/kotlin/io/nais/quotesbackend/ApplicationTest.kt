...
class ApplicationTest {
  ...
  fun testGetQuotes() = testApplication {
    ...
    val jsonElement = kotlinx.serialization.json.Json.parseToJsonElement(responseBodyString)
  ...
  fun testGetSpecificQuote() = testApplication {
    ...
    kotlin.test.assertEquals(
            1,
            actualJsonObject["id"]?.jsonPrimitive?.content?.toInt() ?: error("Missing 'id' field"),
            "ID does not match"
    )
    kotlin.test.assertTrue(
  ...
  fun testPostQuote() = testApplication {
    ...
    kotlin.test.assertEquals(
            "Test Quote",
            actualJsonObject["text"]?.jsonPrimitive?.content ?: error("Missing 'text' field"),
            "Text does not match"
    )
    kotlin.test.assertEquals(
            "Author",
            actualJsonObject["author"]?.jsonPrimitive?.content ?: error("Missing 'author' field"),
    ...
    kotlin.test.assertEquals(
            "Test Quote",
            retrievedJsonObject["text"]?.jsonPrimitive?.content ?: error("Missing 'text' field"),
  ...
  fun testPostQuoteWithMarshalling() = testApplication {
      ...
      install(ClientContentNegotiation) {
        json(
                Json {
                  prettyPrint = true
                  isLenient = true
                  ignoreUnknownKeys = true // Good practice for robustness
                }
        )
      }
    }
    ...
    assertEquals(createdQuote.id, retrievedQuoteById.id, "Retrieved quote ID does not match")
    assertEquals(createdQuote.text, retrievedQuoteById.text, "Retrieved quote text does not match")
    assertEquals(
            createdQuote.author,
  ...
  fun testPostQuoteInvalidFormat() = testApplication {
    ...
    kotlin.test.assertEquals(
            "Failed to convert request body to class io.nais.quotesbackend.Quote",
